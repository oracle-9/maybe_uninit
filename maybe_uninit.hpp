...
/// but without checking whether `T` is destructible.
template <typename T>
concept default_constructible = requires { ::new (static_cast<void*>(std::declval<T*>())) T; };

/// @brief [std::is_constructible](https://en.cppreference.com/w/cpp/types/is_constructible),
/// but without checking whether `T` is destructible.
...
/// @brief [std::is_constructible](https://en.cppreference.com/w/cpp/types/is_constructible),
/// but using braces `{}` instead of parentheses `()` and without checking whether `T` is
/// destructible.
template <typename T, typename... Args>
...
/// @brief Tag type used disambiguate the `maybe_uninit` constructor performing default
/// initialization.
struct default_init_t {};

/// @brief Tag type used disambiguate the `maybe_uninit` constructor performing parentheses
/// initialization, i.e. `T(args...)`.
struct paren_init_t {};

/// @brief Tag type used disambiguate the `maybe_uninit` constructor performing brace
/// initialization, i.e. `T{args...}`.
struct brace_init_t {};

/// @brief Constexpr wrapper of uninitialized values.
/// @details `maybe_uninit` is useful when object construction should be deferred and default
/// construction is either not possible, semantically invalid, or expensive. For example, given the
/// following type:
/// @code {.cpp}
///     struct NonTrivial {
///         NonTrivial() = delete;
...
/// Workarounds include:
/// - allocating the array on the heap with, which adds the runtime overhead of a dynamic allocation
/// and error handling complexity;
/// - using a byte array instead of a `NonTrivial` array and initializing with placement new, which
/// is verbose and error-prone, as it relies on casting, pointer arithmetic, pointer laundering, and
/// proper alignment and size for the byte array.
/// - using a union with a `NonTrivial` member, where the union's default constructor and destructor
...
// Copy/move constructors/assignment operators are already implicitly defined for trivial types, and
// deleted for non-trivial types, as expected.
...
union maybe_uninit {
  public:
    ...
    /// @see `default_init()`
    explicit constexpr maybe_uninit(default_init_t)
    ...
    explicit constexpr maybe_uninit(paren_init_t, Args&&... args)
    ...
    /// @see `brace_init()`
    template <typename... Args>
    explicit constexpr maybe_uninit(brace_init_t, Args&&... args)
    ...
    /// @brief Destructor for trivial `T`s. Destruction is redundant for these types, so defaulting
    /// it trivializes `maybe_uninit` in these cases.
    constexpr ~maybe_uninit()
        requires std::is_trivially_destructible_v<T>
    = default;

    /// @brief Destructor for non-trivial `T`s. Performs no destruction.
    /// @attention It's up to the caller to ensure the object's destructor is invoked if the object
    /// was constructed in the first place.
    constexpr ~maybe_uninit() {}

    /// @brief Default initializes the object.
    /// @returns A reference to the constructed object.
    /// @pre `T` is default constructible as if by `::new (p) T`, where `p` is a `void*` to storage
    /// suitably aligned to hold an object of type `T`.
    /// @note Propagates exceptions thrown by `T`'s default constructor.
    template <typename Self>
    // rvalue-ref to lvalue-ref decay is intentional, to allow taking the address of self.object
    // when self is an rvalue reference.
    ...
    /// @param args Arguments to forward to the constructor of the object.
    /// @returns A reference to the constructed object.
    /// @pre `T` is constructible from @p args as if by `::new (p) T(std::forward<Args>(args)...)`,
    /// where `p` is a `void*` to storage suitably aligned to hold an object of type `T`.
    ...
    /// @p self.
    /// @attention It's up to the caller to ensure accesses to the object through this pointer do
    /// not occur beyond the object's lifetime.
    template <typename Self>
    [[nodiscard]]
    // rvalue-ref to lvalue-ref decay is intentional, to allow taking the address of self.object
    // when self is an rvalue reference.
    ...
    /// @brief Returns a reference to the possibly uninitialized object, preserving the value
    /// category of @p self.
    /// @attention It's up to the caller to ensure accesses to the object through this reference do
    /// not occur beyond the object's lifetime.
    template <typename Self>
    [[nodiscard]]
    ...
    /// @brief Returns a byte span aliased to the possibly uninitialized object, preserving the
    /// constness of `Self` and `T`.
    /// @attention It's up to the caller to ensure accesses to the object representation through
    /// this span do not occur beyond the object's lifetime.
    template <typename Self>
    [[nodiscard]]
    // rvalue-ref to lvalue-ref decay is intentional, to allow taking the address of self.object
    // when self is an rvalue reference.
...
/// @brief Deduction guide that allows type deduction from a single argument, where the deduced
/// `maybe_uninit`'s underlying object type is `T`.
...
constexpr auto paren_init(Args&&... args)
...
/// @relatedalso maybe_uninit
template <detail::sized T>
constexpr auto paren_init(T&& t)
    noexcept(detail::nothrow_paren_constructible_from<std::remove_reference_t<T>, T>)
...
/// @relatedalso maybe_uninit
template <detail::sized T, typename... Args>
constexpr auto brace_init(Args&&... args)
...
/// @relatedalso maybe_uninit
template <detail::sized T>
constexpr auto brace_init(T&& t)
    noexcept(detail::nothrow_brace_constructible_from<std::remove_reference_t<T>, T>)
        -> maybe_uninit<std::remove_reference_t<T>>
